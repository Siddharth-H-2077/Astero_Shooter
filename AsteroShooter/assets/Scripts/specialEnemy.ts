...
@ccclass('specialEnemy')
export class specialEnemy extends Component {
    ...
    private killEnemy()
    {
        setTimeout(()=>
        {
            var pUp=instantiate(this.pickup);
            pUp.setWorldPosition(this.node.getPosition());
            this.node.parent.addChild(pUp);
        },0.1)
        this.node.destroy();
    }
